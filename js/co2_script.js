...
for (ind = 0; ind < items.length/3; ind++) {
  ...
  if (odd ) {
      ...
      drawCL(svgObj, '#cur_line', -cpLength, 0, -cpLength, vStepInt, 0, vStepInt);
  }
  odd = !odd;
}
if (odd) {
...
if (odd) {
  svgObj.circle(30).fill(lineColor).addClass('event_point').move(hStep/2-15, vStep*ind-15);
  svgObj.circle(30).fill(lineColor).addClass('event_point').move(hStep*1.5-15, vStep*ind-15);
} else {
  svgObj.circle(30).fill(lineColor).addClass('event_point').move(hStep*1.5-15, vStep*ind-15);
  svgObj.circle(30).fill(lineColor).addClass('event_point').move(hStep*2.5-15, vStep*ind-15);
}
odd =!odd;
}  
if (odd) {
  svgObj.circle(30).fill(lineColor).addClass('event_point').move(hStep/2-15, vStep*ind-20);
  svgObj.circle(30).fill(lineColor).addClass('event_point').move(hStep*1.5-15, vStep*ind-20);
} else {
  svgObj.circle(30).fill(lineColor).addClass('event_point').move(hStep*1.5-15, vStep*ind-20);
  svgObj.circle(30).fill(lineColor).addClass('event_point').move(hStep*2.5-15, vStep*ind-20);
}  
...
var currentPath = svgObject.find(path).attr("d")[0];
svgObject.find(path).attr("d", currentPath + 'c ' + x1 + ' ' + y1 + ' ' + x2 + ' ' +  y2 + ' ' + x + ' ' + y);
}
